...
class PpoBuffer():
    def __init__(self, buffer_size: int, observation_space: spaces.Space, action_space: spaces.Space,
        ...
        if th.cuda.is_available():
            self.device = th.torch.device('cuda:1')
        else:
            self.device = 'cpu'
    ...
    def add(self,
        ...
        for i in range(self.n_envs):
            self.reward_debugs[i].append(infos[i]['text'])
        '''
        for i in range(self.n_envs):
    ...
    def render(self):
        ...
        vis_idx = np.array([0, 1, 2, 3, 7, 11, 15])
        ...
        for i in range(self.buffer_size):
            ...
            for j in range(self.n_envs):
                ...
                im = cv2.putText(im, txt_1, (w, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 255), 1)
                txt_2 = f'{done} {state_str}'
                im = cv2.putText(im, txt_2, (w, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 255), 1)
                txt_3 = f'rw:{reward:5.2f} rt:{ret:5.2f} a:{advantage:5.2f}'
                im = cv2.putText(im, txt_3, (w, 36), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 255), 1)
                ...
                for i_txt, txt in enumerate(self.reward_debugs[j][i]):
