...
def env_make(port, map_id=0, number_z=5):
    ...
    env_base = gym.make('carla_mzq-v0', host=host, port=port, seed=2022,
        carla_map=map_name, file_path=h5file_path, num_vehicles=30, num_walkers=40)
    # env_base = gym.make('carla_mzq-v0', host=host, port=port, seed=2022)
...
class meta_control(object):
    def __init__(
        ...
        self.gradient_step = 20
        self.batch_size = 256
        self.gamma = 0.99
        self.rollout_step = 100
    ...
    def _build(self, lr):
        ...
        self.optimizer_ter = torch.optim.Adam(self.option_critic.terminations.parameters(), lr=lr)
        ...
        for _ in range(rollout_step):
            ...
            for i in range(n_env):
                ...
                single_a, self._z_deque[i], self._z_deque_tmp[i] = self._sample_single_action(single_ter, single_opt, single_obs, epsilon,
                 skill_env=self.skill_env, z_deque=self._z_deque[i], z_deque_tmp=self._z_deque_tmp[i], done=dones[i])
            ...
            with torch.no_grad():
                option_termination, greedy_option, _ = self.option_critic.predict_option_termination(features, current_option)
    ...
    def _sample_single_action(
        ...
        if current_option == self.number_z:
            ...
            tmp_obs = self._transfer_env(obs=obs, env=skill_env)
            ...
            action = self.skill_robot.forward(observation=tmp_obs, skill=None, z_array=z_array, deterministic=True)
...
def main():
    ...
    argparser.add_argument(
        '--carla_sh_path',
        default='/opt/CARLA_0.9.13/CarlaUE4.sh',
    ...
    try:
        ...
        # prepare wandb
        wandb_cfg = {'wb_project': 'meta_30skill_road&ego_server', 'wb_name': None, 'wb_notes': None, 'wb_tags': None}
        ...
        controller.learn(total_timesteps=2000000)
...
def evaluate_policy(controller: meta_control, video_path: str, min_eval_steps: int = 1000):
    ...
    for _ in range(min_eval_steps):
        ...
        for i in range(controller.env.num_envs):
            ...
            single_a, controller._z_deque[i], controller._z_deque_tmp[i] = controller._sample_single_action(single_ter, single_opt, single_obs, epsilon,
                 skill_env=controller.skill_env, z_deque=controller._z_deque[i], z_deque_tmp=controller._z_deque_tmp[i], done=dones[i])
            action_list.append(single_a)
        action = np.concatenate(action_list, axis=0)
    ...
    pbar.close()
    encoder = ImageEncoder(video_path, list_render[0].shape, 10, 10)
...
def eval():
    ...
    try:
        ...
        evaluate_policy(controller=controller, video_path='meta_control/test_video/test.mp4', min_eval_steps=1000)
