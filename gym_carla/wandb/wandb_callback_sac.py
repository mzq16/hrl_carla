...
class WandbCallback(BaseCallback):
    def __init__(self, cfg, vec_env):
        ...
        self._video_path = Path('sac_data/video')
        self._video_path.mkdir(parents=True, exist_ok=True)
        self._ckpt_dir = Path('sac_data/ckpt')
        self._ckpt_dir.mkdir(parents=True, exist_ok=True)
        self._buffer_dir = Path('sac_data/buffer')
        ...
        self._save_buffer_step = int(5e5)
    ...
    def _on_rollout_end(self):
        ...
        if (self.model.num_timesteps - self._last_time_save_buffer) >= self._save_buffer_step:
            ...
            save_buffer_time = 0
        wandb.log({
            'rollout/reward': self.locals['total_reward'],
            'rollout/value': self.locals['total_value'],
            'rollout/save_buffer_time': save_buffer_time,
            #'rollout/throttle_mean': self.locals['throttle_mean'],
            #'rollout/steer_mean': self.locals['steer_mean'],
            #'rollout/throttle_std': self.locals['throttle_std'],
            #'rollout/steer_std': self.locals['steer_std'],
        
        }, step=self.model.num_timesteps)
        '''
        if (self.model.num_timesteps - self._last_time_buffer) >= self._buffer_step:
            print('start render buffer')
            self._last_time_buffer = self.model.num_timesteps
            ...
            pth_list = os.listdir(self._ckpt_dir)
            for i in range(len(pth_list)):
                index = int(pth_list[i].split('_')[1].split('.')[0])
                if index % 10240 !=0:
                    os.remove((self._ckpt_dir/pth_list[i]).as_posix())
    ...
    @staticmethod
    def evaluate_policy(env, policy, video_path, min_eval_steps=1000, device=torch.device(0)):
