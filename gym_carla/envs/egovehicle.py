...
class EgoVehicle(object):
    ...
    def get_bev_observation(self, distance_threshold, scale_bbox):
        ...
        route_in_pixel = np.array([[self._world_to_pixel(wp.transform.location)]
                                for wp, _ in self._route_trace[0:]])
        ...
        # ev_mask
        '''
        there is a math problem for _get_mask_from_actor_list Function
        use carla.Location() is right for every object
        but for ev, we can use ev_bbox.location instead
        '''
        #ev_mask = self._get_mask_from_actor_list([(ev_transform, ev_bbox.location, ev_bbox.extent)], M_warp)
        
        ev_mask = self._get_mask_from_actor_list([(ev_transform, carla.Location(), ev_bbox.extent)], M_warp)
        # test_mask = self._get_mask_from_actor_list([(ev_transform, ev_bbox.location, ev_bbox.extent)], M_warp)
        # test_n = np.sum(ev_mask==test_mask)
        # print(test_n)
        # time.sleep(1)
