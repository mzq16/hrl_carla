...
class WandbCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        ...
        if (self.model.num_timesteps - self._last_time_eval) >= self._eval_step:
            ...
            avg_ep_stat, ep_events = self.evaluate_policy(
                ...
                min_eval_steps_per_z=100,
                number_timestep=self.model._number_timestep, 
    ...
    @staticmethod
    def evaluate_policy(env, policy, video_path, min_eval_steps_per_z=100, number_timestep=5, number_z=50):
    ...
    @staticmethod
    def _update_z_deque_tmp(dones, env, number_timestep, z_deque, z_deque_tmp):
        z_array = np.zeros((env.num_envs, number_timestep))
        for i in range(env.num_envs):
            if dones[i] is True:
                z_deque[i].clear()
                z_deque[i].append(0)
                z_deque_tmp[i] = deepcopy(z_deque[i])
            assert len(z_deque[i]) != 0, 'z_deque has nothing' 
            assert len(z_deque_tmp[i]) != 0, 'z_deque_tmp has nothing' 
            tmp_z = z_deque[i][-1]
            if tmp_z == z_deque_tmp[i][-1]:
                z_deque_tmp[i].append(tmp_z)
            else:
                z_deque_tmp[i].clear()
                z_deque_tmp[i].append(tmp_z)
            tmp_z_array = np.array(z_deque_tmp[i])

            # padding
            if len(tmp_z_array) != number_timestep:
                pad_len = number_timestep - len(tmp_z_array)
                tmp_z_array = np.pad(tmp_z_array, (0,pad_len), 'constant', constant_values=(0,-1))
                
            assert len(tmp_z_array) == number_timestep, 'the length of tmp_z_array is wrong'
            z_array[i] = tmp_z_array
        return z_array

    @staticmethod
    def _pre_obs(obs, env, number_timestep):
        z_list = obs['z']
        ...
        for i in range(env.num_envs):
            ...
            tmp = tmp_z_list[0]
            len = np.sum(tmp_z_list == tmp)
            if len == number_timestep:
                continue
            else:
                tmp_output[:-(number_timestep - len)] = tmp_output[(number_timestep - len):]
