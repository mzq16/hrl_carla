...
class MapImage(object):
    ...
    @staticmethod
    def draw_map_image(client, world, pixels_per_meter, precision=0.05):
        ...
        trafficlight_manager = trafficlight.Trafficlight(client)
        
        _, _, _, _, list_stopline_vtx = trafficlight_manager.get_all_traffic_light_waypoints(world)

        for stopline_vertices in list_stopline_vtx:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', default='birdview_maps')
    ...
    parser.add_argument('--carla_map', default='Town10HD_Opt')
    ...
    hf_file_path = save_dir / (args.carla_map + '.h5')
    ...
    if hf_file_path.exists():
        ...
        if np.isclose(hf_pixels_per_meter, pixels_per_meter):
            print(f'{args.carla_map}.h5 with pixels_per_meter={pixels_per_meter:.2f} already exists.')

    client = carla.Client("localhost", 2000)
    client.set_timeout(1000)
    print(f'Generating {args.carla_map}.h5 with pixels_per_meter={pixels_per_meter:.2f}.')
    world = client.load_world(args.carla_map)

    dict_masks = MapImage.draw_map_image(client, world, pixels_per_meter)
