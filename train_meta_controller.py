...
def env_make(port, map_id=0, number_z=5):
    ...
    env = RL_meta_Wrapper(env=env_base, number_z=number_z)
    return env
    ...
    def collect_rollout(
        ...
        for _ in range(rollout_step):
            ...
            for i in range(n_env):
                ...
                single_a = self._sample_single_action(single_ter, single_opt, single_obs, epsilon, skill_env=self.skill_env)
            ...
            self._store_transition(
                replay_buffer=self.replay_buffer, buffer_action=greedy_option, new_obs=obs, reward=reward, dones=done, infos=info)
            features = self.option_critic.get_feature(WandbCallback.dict_to_tensor(self._last_obs, self.device))
            with torch.no_grad():
                option_termination, greedy_option = self.option_critic.predict_option_termination(features, current_option)
...
def main():
    ...
    try:
        server_env_configs = diayn_robot.propre_envconfig(num_env=2)
        ...
        if len(env_cfg) == 1:
            env_d = DummyVecEnv([lambda port=port: diayn_robot.env_make(**port) for port in env_cfg])
        else:
            env_d = SubprocVecEnv([lambda port=port: diayn_robot.env_make(**port) for port in env_cfg])

        # prepare wandb
        wandb_cfg = {'wb_project': 'meta_20skill_finetune', 'wb_name': None, 'wb_notes': None, 'wb_tags': None}
        ...
        controller.learn(total_timesteps=800000)
