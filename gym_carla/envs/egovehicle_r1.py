...
class EgoVehicle(object):
    def __init__(self, client, port, multi_cam=False):
        self._actor_list = []
    ...
    def reset(self):
        ...
        self._ego_vehicle = self._spawn_ego_vehicle()
        self._task_handle = Task(self._ego_vehicle, self._client)
    ...
    def get_route(self):
        ...
        while start_locaiton.x==0 and start_locaiton.y==0 and start_locaiton.z==0:
            #print(start_locaiton.x,start_locaiton.y,start_locaiton.z)
            start_locaiton = self._ego_vehicle.get_location()
            self._world.tick()

        time.sleep(0.5)    # wait for ev loc, it is importanf for trace route
        #start_locaiton.x = round(start_locaiton.x)
        #start_locaiton.y = round(start_locaiton.y)
        #print('start_loc', start_locaiton)
        des_list = self._task_handle.gen_task_des()
        #print('des', des_list[0].x, des_list[0].y, des_list[0].z)
    ...
    def set_transform(self, bbox):
        transform_front_mid = carla.Transform(carla.Location(x=0.4*bbox.x, z=1.3*bbox.z), carla.Rotation(pitch=-30.0,yaw=180,roll=0.0))
        transform_front_right = carla.Transform(carla.Location(x=0.9*bbox.x, z=1.3*bbox.z), carla.Rotation(pitch=-10.0,yaw=230,roll=-25.0))
        transform_front_left = carla.Transform(carla.Location(x=0.9*bbox.x, z=1.3*bbox.z), carla.Rotation(pitch=-10.0,yaw=130,roll=25.0))
        transform_back = carla.Transform(carla.Location(x=-1*bbox.x, z=1.3*bbox.z), carla.Rotation(pitch=-30.0,yaw=180,roll=0.0))
        transform_right = carla.Transform(carla.Location(y= 1.1*bbox.y, z=1.3*bbox.z), carla.Rotation(pitch=-30.0,yaw=180,roll=0.0))
        transform_left = carla.Transform(carla.Location(y= -1.1*bbox.y, z=1.3*bbox.z), carla.Rotation(pitch=-30.0,yaw=180,roll=0.0))
    ...
    def _create_camera(self, c_type='rgb'):
        ...
        camera_transform = carla.Transform(carla.Location(x=0.4*bbox.x, z=1.3*bbox.z), carla.Rotation(pitch=-30.0,yaw=180,roll=0.0))
    ...
    def get_bev_observation(self, distance_threshold, scale_bbox):
        ...
        def prepro_route_trace(route_trace, ev_loc, curr_wp_ind):
            if curr_wp_ind >= len(route_trace) - 1:
                return len(route_trace) - 1, 1
            curr_wp, _ = route_trace[curr_wp_ind]
            next_wp, _ = route_trace[curr_wp_ind + 1]
            curr_dist = abs(curr_wp.transform.location.x - ev_loc.x) + abs(curr_wp.transform.location.y - ev_loc.y)
            next_dist = abs(next_wp.transform.location.x - ev_loc.x) + abs(next_wp.transform.location.y - ev_loc.y)
            if next_dist < curr_dist:
                curr_wp_ind += 1
            return curr_wp_ind, len(route_trace) - curr_wp_ind
        curr_wp_ind, rest_wp_number = prepro_route_trace(self._route_trace, ev_loc, self._start_wp_ind)
        self._start_wp_ind = curr_wp_ind
        rest_wp_number = min(30, rest_wp_number)
        route_in_pixel = np.array([[self._world_to_pixel(wp.transform.location)]
                                for wp, _ in self._route_trace[curr_wp_ind : curr_wp_ind+rest_wp_number]])
        ...
        obs_dict = {'rendered': image, 'masks': masks, 'ev_history_masks': ev_histroy_masks}
