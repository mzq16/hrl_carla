...
class my_diayn(BaseAlgorithm):
    ...
    def __init__(
        ...
        self._z_deque_tmp = [deque(maxlen=number_timestep) for i in range(env.num_envs)]
    ...
    def _update_z_deque_tmp(self, dones):
        z_array = np.zeros((self.env.num_envs, self._number_timestep))
    ...
    def _pre_obs(self, obs):
        ...
        for i in range(self.env.num_envs):
            ...
            if len == self._number_timestep:
                continue
            else:
                tmp_output[:-(self._number_timestep - len)] = tmp_output[(self._number_timestep - len):]
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            # revise behavior z 
            for i in range(self.env.num_envs):
                self._z_deque[i].append(tmp_z)
            z_array = self._update_z_deque_tmp(dones)
            new_obs['z'] = z_array
            
            new_obs = self._pre_obs(obs=new_obs)
    ...
    def load(
        ...
        **kwargs,
        ...
        #model.log_ent_coef = th.log(th.ones(1) * 0.1).requires_grad_(False)
        #model.ent_coef_optimizer = th.optim.Adam([model.log_ent_coef], lr=5e-5)
        if train_bool:
            if buffer_path:
