...
class my_diayn(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if self.replay_buffer_list is None:
            self.replay_buffer_list: List[Union[ReplayBuffer, DictReplayBuffer]] = []
            single_buffer_size = int(self.buffer_size / self._number_z)
        ...
        #self.policy.to_cuda(self.device)

        #self._create_aliases()
        self.actor_list = self.policy.actor_list
        self.critic_list = self.policy.critic_list
        self.critic_target_list = self.policy.critic_target_list
    ...
    def collect_rollouts(
        ...
        replay_buffer_list: ReplayBuffer,
        action_noise: Optional[ActionNoise] = None,
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            if self.use_sde and self.sde_sample_freq > 0 and num_collected_steps % self.sde_sample_freq == 0:
                ...
                assert len(self.actor_list) == self._number_z, "the number of actor must be consistant"
                for i in range(self._number_z):
                    actor = self.actor_list[i]
                    actor.reset_noise(env.num_envs)

            # Select action randomly or according to policy
            actions, buffer_actions = self._sample_action(learning_starts, action_noise, env.num_envs, tmp_z)
            ...
            replay_buffer = replay_buffer_list[tmp_z]
    ...
    def train(self, gradient_steps: int, callback: BaseCallback, batch_size: int = 64):
        ...
        for i in range(self._number_z):
            ...
            optimizers.append(actor.optimizer)
            optimizers.append(critic.optimizer)
        ...
        for gradient_step in range(gradient_steps):
            ...
            tmp_z = np.random.randint(0, self._number_z)
            replay_buffer = self.replay_buffer_list[tmp_z]
            # check buffer pos
            
            while replay_buffer.pos == 0 and not replay_buffer.full:
                tmp_z = np.random.randint(0, self._number_z)
                replay_buffer = self.replay_buffer_list[tmp_z]
            actor = self.actor_list[tmp_z]
            critic = self.critic_list[tmp_z]
            critic_target = self.critic_target_list[tmp_z]
            ...
            q_values_pi = th.cat(critic.forward(replay_data.observations, actions_pi), dim=1)
    ...
    def predict_multi(
        ...
        return self.policy.predict_multi(observation=observation, deterministic=deterministic, index_actor=index_actor)
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            unscaled_action = self.predict_multi(self._last_obs, deterministic=False, index_actor=index_actor)
    ...
    def save_replay_buffer(self, path: Union[str, pathlib.Path, io.BufferedIOBase]) -> None:
        ...
        for i in range(self._number_z):
            replay_buffer = self.replay_buffer_list[i]
            output_path = split_path[0] + '_{}.'.format(i) + split_path[1]
            save_to_pkl(output_path, replay_buffer, self.verbose)
    ...
    def load_replay_buffer(
        ...
        buffer_list = os.listdir(path)
        assert len(buffer_list) == number_z, "the number of buffer and the number of actors must be consistent"
        buffer_list.sort(key=lambda x: int(x.split('_')[1].split('.')[0]))
        self.replay_buffer_list = []
        for i in range(number_z):
            input_path = os.path.join(path, buffer_list[i])
            replay_buffer = load_from_pkl(input_path, self.verbose)
            ...
            if not hasattr(replay_buffer, "handle_timeout_termination"):  # pragma: no cover
                ...
                replay_buffer.timeouts = np.zeros_like(self.replay_buffer.dones)
    ...
    def load(
        ...
        **kwargs,
        ...
        if buffer_path:
            model.load_replay_buffer(buffer_path, model._number_z)
        model._setup_model()
